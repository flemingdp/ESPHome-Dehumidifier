...
class TestMideaDehum : public esphome::midea_dehum::MideaDehumComponent {
 public:
  ...
  float   raw_timer_hours() const { return this->last_timer_hours_; }
#endif
...
// MAD50P1AWS (manufactured May 2024) short V1 status frames captured from the
// live appliance with its factory USB module connected. These use 0x05/0xA0
// instead of V1's legacy byte-10 0xC8 discriminator.
static const uint8_t MAD50P1AWS_STATUS_HUM35[] = {0xAA, 0x22, 0xA1, 0x00, 0x00, 0x00, 0x00, 0x00, 0x03, 0x05, 0xA0, 0x01, 0x01, 0x50, 0x7F, 0x7F, 0x00, 0x23, 0x00, 0x18, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x3A, 0x5A, 0x00, 0x00, 0x00, 0x00, 0x2F, 0x28, 0x1F};
static const uint8_t MAD50P1AWS_DEVICE_ACK[] = {0xAA, 0x2A, 0xA1, 0x00, 0x00, 0x00, 0x00, 0x00, 0x03, 0x07, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x4B};
static const uint8_t MAD50P1AWS_STATUS_HUM40[] = {0xAA, 0x22, 0xA1, 0x00, 0x00, 0x00, 0x00, 0x00, 0x03, 0x05, 0xA0, 0x01, 0x01, 0x50, 0x7F, 0x7F, 0x00, 0x28, 0x00, 0x18, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x3A, 0x5A, 0x00, 0x00, 0x00, 0x00, 0x2F, 0xA1, 0xA1};
static const uint8_t MAD50P1AWS_STATUS_HUM45_OFF[] = {0xAA, 0x22, 0xA1, 0x00, 0x00, 0x00, 0x00, 0x00, 0x03, 0x05, 0xA0, 0x00, 0x01, 0x50, 0x7F, 0x7F, 0x00, 0x2D, 0x00, 0x18, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x3A, 0x5A, 0x00, 0x00, 0x00, 0x00, 0x2F, 0xB8, 0x86};
static const uint8_t MAD50P1AWS_STATUS_HUM50[] = {0xAA, 0x22, 0xA1, 0x00, 0x00, 0x00, 0x00, 0x00, 0x03, 0x05, 0xA0, 0x01, 0x01, 0x50, 0x7F, 0x7F, 0x00, 0x32, 0x00, 0x18, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x3A, 0x5A, 0x00, 0x00, 0x00, 0x00, 0x2F, 0xAC, 0x8C};
...
// V2 status — OFF, mode=3, fan=40, set=50%, humidity=45%, temp=23.3C, 2h ON timer (byte14=0x88)
static const uint8_t V2_STATUS_TIMER2H[] = {0xAA, 0x23, 0xA1, 0x00, 0x00, 0x00, 0x00, 0x00, 0x08, 0x05, 0xA0, 0x00, 0x03, 0x28, 0x88, 0x7F, 0x00, 0x32, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x2D, 0x5F, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x97};

// ══════════════════════════════════════════════════════════════════════════
//  V1.3 (agreement 0x03) MCU frames — from Midea cube 35 MC35MSWBA3RCM (might be a Canada specific SKU)
//  These devices use agreement version 0x03 in frame header byte[8] across
//  all frames (ACK, seed status, polled status, push-on-change).  Before the
//  adaptive status-query fix, the pre-built queries with hardcoded agreement
//  versions failed to elicit poll responses.
// ══════════════════════════════════════════════════════════════════════════

// V1.3 device ACK — agreement 0x00, MCU protocol version 0x00 (V1-style).
// The MCU uses agreement 0x00 in the ACK but 0x03 in status frames.
static const uint8_t V1_3_DEVICE_ACK[] = {0xAA, 0x0B, 0xA1, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x07, 0x00, 0x53};

// V1.3 seed status (0x05/0xA0) — the frame @dnakamura's device sends after
// the ACK+ping handshake phase.  V1 treats 0x05 as a UART ping and echoes
// it back, then marks handshake done.  V2 treats 0x05/0xA0 as a status frame.
// State: ON, mode=2(Continuous), fan=40, set=50%, humidity=41%, temp=22.0°C
static const uint8_t V1_3_SEED_STATUS[] = {0xAA, 0x23, 0xA1, 0x00, 0x00, 0x00, 0x00, 0x00, 0x03, 0x05, 0xA0, 0x01, 0x02, 0x28, 0x7F, 0x7F, 0x00, 0x32, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x29, 0x5E, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

// V1.3 polled status (0x03/0xC8) — reply to the 0x03 status query.
// State: ON, mode=2(Continuous), fan=40, set=50%, humidity=41%, temp=22.0°C
static const uint8_t V1_3_POLL_STATUS[] = {0xAA, 0x23, 0xA1, 0x00, 0x00, 0x00, 0x00, 0x00, 0x03, 0x03, 0xC8, 0x01, 0x02, 0x28, 0x7F, 0x7F, 0x00, 0x32, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x29, 0x5E, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

// V1.3 push-on-change (0x02/0xC8) — unsolicited status sent when physical
// buttons are pressed.  State: ON, mode=1(Setpoint), fan=40, set=60%,
// humidity=55%, temp=23.3°C
static const uint8_t V1_3_PUSH_STATUS[] = {0xAA, 0x23, 0xA1, 0x00, 0x00, 0x00, 0x00, 0x00, 0x03, 0x02, 0xC8, 0x01, 0x01, 0x28, 0x7F, 0x7F, 0x00, 0x3C, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x37, 0x5F, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
